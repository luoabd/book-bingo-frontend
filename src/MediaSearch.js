...
function MediaSearch({ id, metaData, stateChanger, returnCover }) {
  ...
  const loadOptions = (inputValue) => {
    async function fetchSearchResults() {
      const response = await fetch(`${URL}/media/search?q=${inputValue}`);
      const media = await response.json();
      let arr = Object.keys(media).map((k) => media[k]);
      return arr;
    }
